...
/**
 * This class contains all of the code that makes the Mine Sweeper game
 * function. The user is first prompted to specify how big they want the game to
 * be. Then, the arena is populated with mines and the user is asked to choose a
 * location to sweep. If there is a mine at the location, the game is over and
 * the user is asked if they want to play again. If there is not a mine, the
 * number of mines nearby gets printed out at the location the user choose to
 * sweep. If there are no mines nearby, the program sweeps the surrounding mines
 * to see how many mines there are. If the user can choose all the locations
 * where there is no mine, they win. We call the Config class whenever we need a
 * constant value.
 * 
 * 
 * @author Vinnie Angellotti
...
public class MineSweeper {
	...
	public static void main(String[] args) {
		...
		do {
			...
			int userWidth = promptUser(sc, userWidthPrompt, Config.MIN_SIZE, Config.MAX_SIZE);// calls promptUser and
																								// gets
			// the user's desired width
			int userHeight = promptUser(sc, userHeightPrompt, Config.MIN_SIZE, Config.MAX_SIZE);// calls promptUser and
																								// gets
			// the user's desired height
			boolean[][] mineAtLoc = new boolean[userHeight][userWidth];// array that has info about where bombs are
			int numMines = placeMines(mineAtLoc, rand);// calls placeMines to see how many total mines there are in
														// array
			System.out.println();
			...
			eraseMap(mines);// starts map "fresh" with all mines being unswept
			printMap(mines);
			...
			do {// this loop will make this whole program keep going until the user hits
				...
				rowSwept = promptUser(sc, rowSweep, 1, userHeight);// calls promptUser and gets a row the user wants
																	// swept
				rowSwept = rowSwept - 1;// makes rowSwept 0 based
				String colSweep = "column: ";// tells the user to put a column they want swept
				colSwept = promptUser(sc, colSweep, 1, userWidth);// calls promptUser and gets a column the user wants
																	// swept
				colSwept = colSwept - 1;// makes colSwept 0 based

				if (mineAtLoc[rowSwept][colSwept] == true) {// there is a mine at the location
					// System.out.println("Sorry you lost");
					showMines(mines, mineAtLoc);
					mines[rowSwept][colSwept] = Config.SWEPT_MINE;// prints special character where the bomb was that
																	// you choose
					printMap(mines);
					System.out.println("Sorry, you lost.");
					break;// skips rest of loop and goes to the part where we ask user if they want to
							// play again
				} else if (mineAtLoc[rowSwept][colSwept] == false) {// there is not a mine at the location

					int minesTouching = sweepLocation(mines, mineAtLoc, rowSwept, colSwept);// finds how many mines are
																							// touching
					if (minesTouching == 0) {// if there are no mines nearby
						sweepAllNeighbors(mines, mineAtLoc, rowSwept, colSwept);
					}
					if (allSafeLocationsSwept(mines, mineAtLoc) == true) {// if the user has swept all locations with a
																			// mine
						showMines(mines, mineAtLoc);// reveals all the hidden mines
						printMap(mines);
			...
			} while (allSafeLocationsSwept(mines, mineAtLoc) == false || mineAtLoc[rowSwept][colSwept] == false);
	...
	public static int promptUser(Scanner in, String prompt, int min, int max) {
		...
		while (true) {
			if (!in.hasNextInt()) {// makes sure the user input is a number
				in.nextLine();
		...
		for (int i = 0; i <= maxRow; i++) {
			for (int j = 0; j <= maxCol; j++) {
				if (i == 0 && j == 0) {
					...
					System.out.print(" " + map[i - 1][j - 1]);// prints array out inside the new outline of map
				}
			}
			System.out.println();
		}
	...
	public static int placeMines(boolean[][] mines, Random randGen) {
		...
		for (int i = 0; i < mines.length; i++) {
			for (int j = 0; j < mines[i].length; j++) {
				if (randGen.nextFloat() < Config.MINE_PROBABILITY) {// places mine where the random number is less than
																	// a probability defined in Config
					mines[i][j] = true;
					if (mines[i][j] == true) {// adds how many total mines there are
						numMines += 1;
					}
				}
	...
	public static int numNearbyMines(boolean[][] mines, int row, int col) {
		...
		for (int i = row - 1; i <= row + 1; i++) {
			for (int j = col - 1; j <= col + 1; j++) {
				if (i == row && j == col) {// doesn't sweep location user entered
					continue;
				}
				if (i < 0 || j < 0 || i >= mines.length || j >= mines[i].length)// makes sure we are calling part of the
																				// array that is in the array's bounds
					continue;
				if (mines[i][j]) {
					numOfMinesTouch++;
				}
	...
	public static boolean allSafeLocationsSwept(char[][] map, boolean[][] mines) {
		...
		for (int i = 0; i < map.length; i++) {
			for (int j = 0; j < map[0].length; j++) {
				if (mines[i][j] == false) {
					if (map[i][j] == Config.UNSWEPT) {// if there is an unswept location all safe locations are not
														// swept
						return false;
	...
	public static int sweepLocation(char[][] map, boolean[][] mines, int row, int col) {
		...
		if (mines[row][col] == false) {// no mine
			...
			if (numMinesNearby > 0) {
				String tempString = Integer.toString(numMinesNearby);// parses string
				char tempChar = tempString.charAt(0);// turns string into character
				map[row][col] = tempChar;// prints the number of mines nearby in the place the user swept
	...
	public static void sweepAllNeighbors(char[][] map, boolean[][] mines, int row, int col) {
		...
		for (i = row - 1; i <= row + 1; i++) {
			for (j = col - 1; j <= col + 1; j++) {
				...
				sweepLocation(map, mines, i, j);// calls sweep location for all neighbors
			}
		}
		return;
	}
